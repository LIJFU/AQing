...
public class ServerTaskService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_PUB_BLOG_COMMENT.equals(action)) {
            ...
            com.xiaoyuan54.child.edu.app.bean.Tweet tweet = intent.getParcelableExtra(BUNDLE_PUB_TWEET_TASK);
            if (tweet != null) {
                pubTweet(tweet);
            }
        } else if (ACTION_PUB_SOFTWARE_TWEET.equals(action)) {
            com.xiaoyuan54.child.edu.app.bean.Tweet tweet = intent
    ...
    private void publicBlogComment(final PublicCommentTask task) {
        ...
        HttpClientApi.publicBlogComment(task.getId(), task.getUid(), task
    ...
    private void publicComment(final PublicCommentTask task) {
        ...
        HttpClientApi.publicComment(task.getCatalog(), task.getId(),
    ...
    // }
    //
    private void pubTweet(final com.xiaoyuan54.child.edu.app.bean.Tweet tweet) {
        ...
        HttpClientApi.pubTweet(tweet, new PublicTweetResponseHandler(
                getMainLooper(), tweet, KEY_TWEET));
    }

    private void pubSoftWareTweet(final com.xiaoyuan54.child.edu.app.bean.Tweet tweet, int softid) {
        tweet.setId((int) System.currentTimeMillis());
        ...
        notifySimpleNotifycation(id, getString(R.string.tweet_publishing),
                getString(R.string.tweet_public),
                getString(R.string.tweet_publishing), true, false);
        HttpClientApi.pubSoftwareTweet(tweet.getBody(), new PublicTweetResponseHandler(getMainLooper(), tweet, KEY_SOFTWARE_TWEET));
    }
