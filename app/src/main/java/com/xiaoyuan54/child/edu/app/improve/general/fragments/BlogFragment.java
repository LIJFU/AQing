...
import com.xiaoyuan54.child.edu.app.improve.general.adapter.BlogActionAdapter;
import com.xiaoyuan54.child.edu.app.improve.general.adapter.BlogAdapter;
...
public class BlogFragment extends BaseGeneralListFragment<Blog> {
    ...
    private void displacementCatalog(int position) {
        switch (position) {
            case 0:
                catalog = HttpClientApi.CATALOG_BLOG_RECOMMEND;
                break;
            case 1:
                catalog = HttpClientApi.CATALOG_BLOG_HEAT;
                break;
            case 2:
                catalog = HttpClientApi.CATALOG_BLOG_NORMAL;
                break;
        }
    }
    ...
    private void requestLocalCache() {
        verifyCacheType();
        mBean = (ProPageBean<Blog>) CacheManager.readObject(getActivity(), CACHE_NAME);
        if (mBean != null) {
            mAdapter.clear();
