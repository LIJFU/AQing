...
import com.xiaoyuan54.child.edu.app.bean.BlogCommentList;
...
public class CommentFrament extends BaseListFragment<Comment> implements
    ...
    protected void sendRequestData() {
        if (mIsBlogComment) {
            HttpClientApi.getBlogCommentList(mId, mCurrentPage, mHandler);
        } else {
            HttpClientApi.getCommentList(mId, mCatalog, mCurrentPage, mHandler);
        }
    }
    ...
    private void handleDeleteComment(Comment comment) {
        ...
        if (mIsBlogComment) {
            HttpClientApi.deleteBlogComment(
    ...
    private void sendReply(String text) {
        ...
        if (mIsBlogComment) {
            HttpClientApi.publicBlogComment(mId, AppContext.getInstance()
                    .getLoginUid(), text, mCommentHandler);
        } else {
            HttpClientApi.publicComment(mCatalog, mId, AppContext.getInstance()
    ...
    private void handleReplyComment(Comment comment, String text) {
        ...
        if (mIsBlogComment) {
            HttpClientApi.replyBlogComment(mId, AppContext.getInstance()
                    .getLoginUid(), text, comment.getId(), comment
                    .getAuthorId(), mCommentHandler);
        } else {
            HttpClientApi.replyComment(mId, mCatalog, comment.getId(), comment
