...
import com.xiaoyuan54.child.edu.app.improve.tweet.fragments.TweetFragment;
...
import com.xiaoyuan54.child.edu.app.util.CyptoUtils;
...
import static com.xiaoyuan54.child.edu.app.AppConfig.KEY_FRITST_START;
import static com.xiaoyuan54.child.edu.app.AppConfig.KEY_LOAD_IMAGE;
import static com.xiaoyuan54.child.edu.app.AppConfig.KEY_NIGHT_MODE_SWITCH;
import static com.xiaoyuan54.child.edu.app.AppConfig.KEY_TWEET_DRAFT;
...
public class AppContext extends BaseApplication
{
    ...
    public static com.xiaoyuan54.child.edu.app.bean.user.User appUser;
    ...
    private void init()
    {
        ...
        // Bitmap缓存地址
        HttpConfig.CACHEPATH = AppDirManager.CACHE;
    }
    ...
    public void saveUserInfo(final User user) {
        ...
        setProperties(new Properties() {
            {
                setProperty("user.user_name", String.valueOf(user.getId()));
                setProperty("user.nick_name", user.getName());
                setProperty("user.gender", user.getPortrait());// 用户头像-文件名
                setProperty("user.password", user.getAccount());
                setProperty("user.mobile",
                        CyptoUtils.encode("oschinaApp", user.getPwd()));
                setProperty("user.header_img", user.getLocation());
                setProperty("user.birthday", String.valueOf(user.getFollowers()));
                setProperty("user.score", String.valueOf(user.getFans()));
                setProperty("user.coin", String.valueOf(user.getScore()));
                setProperty("user.vip",
                        String.valueOf(user.getFavoritecount()));
                setProperty("user.email", String.valueOf(user.getGender()));
                setProperty("user.province",
                        String.valueOf(user.isRememberMe()));
                setProperty("user.city",
                        String.valueOf(user.isRememberMe()));
                setProperty("user.address",
                        String.valueOf(user.isRememberMe()));
                setProperty("user.wechat_uuid",
                        String.valueOf(user.isRememberMe()));
                setProperty("user.wechat_uuid",
                        String.valueOf(user.isRememberMe()));
            }
