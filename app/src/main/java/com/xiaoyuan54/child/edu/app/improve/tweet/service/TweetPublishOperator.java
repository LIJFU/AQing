package com.xiaoyuan54.child.edu.app.improve.tweet.service;
...
import com.xiaoyuan54.child.edu.app.improve.bean.resource.ImageResource;
...
class TweetPublishOperator implements Runnable, Contract.IOperator {
    ...
    private void uploadImages(final int index, final String token, final String[] paths, final UploadImageCallback runnable) {
        ...
        HttpClientApi.uploadImage(token, path, new LopperResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
    ...
    private void publish() {
        HttpClientApi.pubTweet(model.getContent(), model.getCacheImagesToken(), null, new LopperResponseHandler() {
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                String error = "";
                String response = responseString == null ? "" : responseString;
