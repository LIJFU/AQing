...
import com.xiaoyuan54.child.edu.app.improve.base.adapter.BaseGeneralRecyclerAdapter;
    ...
    public void initData() {
        ...
        mHandler = new TextHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, String responseString) {
                try {
                    Type type = getType();
                    PageBean result = AppContext.createGson().fromJson(responseString, type);
                    if (result.isSuccess() &&null!= result.getItem())
                    {
                        setListData(result);
                        onRequestSuccess(result.getStacode());
                    }
                    else
                    {
    ...
    protected void setListData(PageBean<T> resultBean) {
        ...
        if (mIsRefresh) {
           ...
           // mBean.setPrevPageToken((resultBean == null ? null : resultBean.getResult().getPrevPageToken()));
            ...
            mAdapter.addAll(resultBean.getItem());
        }

        mAdapter.setState(resultBean.getItem() == null || resultBean.getItem().size() < 20 ? BaseRecyclerAdapter.STATE_NO_MORE : BaseRecyclerAdapter.STATE_LOADING, true);
