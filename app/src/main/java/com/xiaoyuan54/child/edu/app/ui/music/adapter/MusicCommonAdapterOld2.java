 ...
 * 儿歌通用fragment通用适配器
 */
public class MusicCommonAdapterOld2 extends BaseListAdapter<Music> {
    ...
    public MusicCommonAdapterOld2(Callback callback) {
        super(callback);
    }

    @Override
    protected int getLayoutId(int position, Music item) {
        return R.layout.fragment_item_music_common;
    }

    @Override
    protected void convert(ViewHolder vh, Music item, int position)
    {
        String authorPortrait = item.getCoverImg();
        ...
        int downTime  =  item.getDownTimes();
        if(downTime>0)
        {
            content.setText("下载"+downTime+"次");
