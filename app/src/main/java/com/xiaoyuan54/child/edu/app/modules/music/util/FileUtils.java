...
public class FileUtils {
    ...
    public static List<Song> musicFiles(File dir)
    {
        ...
        if (musics.size() > 1) {
            Collections.sort(musics, new Comparator<Song>() {
                @Override
    ...
    public static Song fileToMusic(File file) {
        ...
        song.setTitle(title);
        song.setSubTitle(displayName);
        song.setSinger(artist);
        song.setSongUrl(file.getAbsolutePath());
        song.setCoverImg(album);
