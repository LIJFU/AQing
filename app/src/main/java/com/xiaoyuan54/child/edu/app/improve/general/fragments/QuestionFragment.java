...
import com.xiaoyuan54.child.edu.app.improve.general.adapter.QuesActionAdapter;
import com.xiaoyuan54.child.edu.app.improve.general.adapter.QuestionAdapter;
...
public class QuestionFragment extends BaseGeneralListFragment<Question> {
    ...
    private void requestLocalCache() {
        verifyCacheType();
        mBean = (ProPageBean<Question>) CacheManager.readObject(getActivity(), CACHE_NAME);
        if (mBean != null) {
            mAdapter.clear();
    ...
    protected void requestData() {
        super.requestData();
        verifyCacheType();
        HttpClientApi.getQuestionList(catalog, null, mHandler);
    }

    @Override
    protected Type getType() {
        return new TypeToken<ResultBean<ProPageBean<Question>>>() {
        }.getType();
    }

    @Override
    protected void setListData(ResultBean<ProPageBean<Question>> resultBean) {
        verifyCacheType();
