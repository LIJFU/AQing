...
public class CommentExsActivity extends BaseBackActivity {
    ...
    private ProPageBean<CommentEX> mPageBean;
    ...
    private void handleSendComment(long id, final long commentId, long commentAuthorId, String content) {
        ...
        HttpClientApi.pubQuestionComment(id, commentId, commentAuthorId, content, new TextHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
    ...
    private void getData(final boolean clearData, String token) {
        HttpClientApi.getComments(mId, mType, "refer", token, new TextHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, String responseString) {
                try {
                    Type type = new TypeToken<ResultBean<ProPageBean<CommentEX>>>() {
                    }.getType();

                    ResultBean<ProPageBean<CommentEX>> resultBean = AppContext.createGson().fromJson(responseString, type);
