package com.xiaoyuan54.child.edu.app.improve.comment;
...
import com.xiaoyuan54.child.edu.app.improve.widget.RecyclerRefreshLayout;
...
public class CommentsActivity extends BaseBackActivity {
    ...
    private ProPageBean<Comment> mPageBean;
    ...
    private void handleSendComment(int type, long id, final long commentId, long commentAuthorId, String content) {
        ...
        switch (type) {
            case 2:
                HttpClientApi.pubQuestionComment(id, commentId, commentAuthorId, content, mHandler);
                break;
            case 3:
                HttpClientApi.pubBlogComment(id, commentId, commentAuthorId, content, mHandler);
                break;
            case 4:
                HttpClientApi.pubTranslateComment(id, commentId, commentAuthorId, content, mHandler);
                break;
            case 6:
                HttpClientApi.pubNewsComment(id, commentId, commentAuthorId, content, mHandler);
                break;
            default:
                break;
        }
    ...
    private void getData(final boolean clearData, String token) {
        HttpClientApi.getComments(mId, mType, "refer", token, new TextHttpResponseHandler() {
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
            ...
            public void onSuccess(int statusCode, Header[] headers, String responseString) {
                try {
                    Type type = new TypeToken<ResultBean<ProPageBean<Comment>>>() {
                    }.getType();

                    ResultBean<ProPageBean<Comment>> resultBean = AppContext.createGson().fromJson(responseString, type);
